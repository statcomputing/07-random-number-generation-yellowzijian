...
$$g(x)=\frac{2\Gamma(\theta)}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}Gamma(\theta,1) + \frac{\Gamma(\theta+\frac{1}{2})}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}Gamma(\theta+\frac{1}{2},1)$$
Therefore, the component distributions are $Gamma(\theta,1)$ and $Gamma(\theta+\frac{1}{2},1)$ and their weights in the mixture are $\frac{2\Gamma(\theta)}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}$ and $\frac{\Gamma(\theta+\frac{1}{2})}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}$ respectively.
...
sample_g <- function(theta){
  ...
  w1 <- 2 * c * gamma(theta); w2 <- c * gamma(theta + 0.5)
  u <- rbinom(n, size = 1, prob = w1)
  x <- rgamma(n, shape = ifelse(u == 1, par1, par2), scale = 1) 
  g <- w1 * dgamma(x, shape = par1, scale = 1) +
  w2 * dgamma(x, shape = par2, scale = 1)
...
They are almost the same for $\theta=2,3,4,5,6$ respectively.
